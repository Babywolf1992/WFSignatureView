...
#define             STROKE_WIDTH_MIN 0.001
#define             STROKE_WIDTH_MAX 0.02
#define       STROKE_WIDTH_SMOOTHING 0.2

#define           VELOCITY_CLAMP_MIN 20
#define           VELOCITY_CLAMP_MAX 5000

#define QUADRATIC_DISTANCE_TOLERANCE 5.0
...
struct WFSignaturePoint
{
    GLKVector3		vertex;
    GLKVector3		color;
};
typedef struct WFSignaturePoint WFSignaturePoint;


static const int maxLength = MAXIMUM_VERTECES;


static inline void addVertex(uint *length, NSMutableArray *pointsArray, WFSignaturePoint v) {
    uint sum = 0;
    for (NSNumber *num in pointsArray) {
        sum += (NSUInteger)[num integerValue];
    }
    uint len = *length + sum;
    if (len >= maxLength) {
        return;
    ...
    memcpy(data + sizeof(WFSignaturePoint) * len, &v, sizeof(WFSignaturePoint));
...
static GLKVector3 perpendicular(WFSignaturePoint p1, WFSignaturePoint p2) {
    GLKVector3 ret;
...
static WFSignaturePoint ViewPointToGL(CGPoint viewPoint, CGRect bounds, GLKVector3 color) {
    
    return (WFSignaturePoint) {
        {
...
@interface WFSignatureView () {
    ...
    WFSignaturePoint SignatureVertexData[maxLength];
    uint length;
    
    WFSignaturePoint SignatureDotsData[maxLength];
    uint dotsLength;
    ...
    WFSignaturePoint previousVertex;
    WFSignaturePoint currentVelocity;
}
...
- (void)commonInit {
    ...
    GLKVector3 color = { 0, 0, 0 };
    self.penColor = color;
    
    if (context) {
        time(NULL);
        _currentPath = 0;
        pointsArray = [[NSMutableArray alloc] initWithCapacity:10];
        pathsArray  = [[NSMutableArray alloc] initWithCapacity:10];
        sizesArray = [[NSMutableArray alloc] initWithCapacity:10];
        colorsArray = [[NSMutableArray alloc] initWithCapacity:10];
...
- (void)tap:(UITapGestureRecognizer *)t {
    ...
    if (t.state == UIGestureRecognizerStateRecognized) {
        ...
        WFSignaturePoint touchPoint = ViewPointToGL(l, self.bounds, self.penColor);
        addVertex(&dotsLength, pointsArray, touchPoint);
        
        WFSignaturePoint centerPoint = touchPoint;
        centerPoint.color = StrokeColor;
        addVertex(&dotsLength, pointsArray, centerPoint);
...
- (void)pan:(UIPanGestureRecognizer *)p {
    ...
    float width_max = STROKE_WIDTH_MAX + self.width*0.015;
    float width_min = STROKE_WIDTH_MIN + self.width*0.001;
    ...
    float newThickness = (width_max - width_min) * (1 - normalizedVelocity) + width_min;
    ...
    if ([p state] == UIGestureRecognizerStateBegan) {
        ...
        WFSignaturePoint startPoint = ViewPointToGL(l, self.bounds, self.penColor);
        previousVertex = startPoint;
        previousThickness = penThickness;
        
        
        addVertex(&length, pointsArray, startPoint);
        addVertex(&length, pointsArray, previousVertex);
        
        self.hasSignature = YES;
        //        NSLog(@"%d",pointsArray.count);
        self.currentPath = (int)pointsArray.count;
        ...
        if (distance > QUADRATIC_DISTANCE_TOLERANCE) {
            ...
            for (i = 0; i < segments; i++)
            {
                ...
                WFSignaturePoint v = ViewPointToGL(quadPoint, self.bounds, self.penColor);
            ...
            WFSignaturePoint v = ViewPointToGL(l, self.bounds, self.penColor);
            [self addTriangleStripPointsForPrevious:previousVertex next:v];
        ...
        WFSignaturePoint v = ViewPointToGL(l, self.bounds, self.penColor);
        addVertex(&length, pointsArray, v);
        
        previousVertex = v;
        addVertex(&length, pointsArray, previousVertex);
        [pointsArray addObject:[NSNumber numberWithInt:length]];
        length = 0;
    }
...
- (void)bindShaderAttributes {
    ...
    glVertexAttribPointer(GLKVertexAttribPosition, 3, GL_FLOAT, GL_FALSE, sizeof(WFSignaturePoint), 0);
...
- (void)setupGL
{
    ...
    length = 0;
    penThickness = 0.003;
    if (penThickness<0.003 || penThickness>0.008) {
        penThickness = 0.003;
    }
    self.width = 1;
...
- (void)addTriangleStripPointsForPrevious:(WFSignaturePoint)previous next:(WFSignaturePoint)next {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        WFSignaturePoint stripPoint = {
            { p1.x + difX, p1.y + difY, 0.0 },
            self.penColor
        };
        addVertex(&length, pointsArray, stripPoint);
